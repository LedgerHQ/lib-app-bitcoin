...
bool swap_copy_transaction_parameters(create_transaction_parameters_t *params) {
  ...
  if (swap_validated
          .destination_address[sizeof(swap_validated.destination_address) -
                               1] != '\0') {
    return false;
  }

  // store amount as big endian in 8 bytes, so the passed data should be
  // alligned to right input {0xEE, 0x00, 0xFF} should be stored like {0x00,
  // 0x00, 0x00, 0x00, 0x00, 0xEE, 0x00, 0xFF}
  memcpy(swap_validated.amount + 8 - params->amount_length, params->amount,
         params->amount_length);
  memcpy(swap_validated.fees + 8 - params->fee_amount_length,
         params->fee_amount, params->fee_amount_length);

  // Save amount and fees
  //    swap_str_to_u64(params->amount, params->amount_length,
  //    &swap_validated.amount); swap_str_to_u64(params->fee_amount,
  //    params->fee_amount_length, &swap_validated.fees);
  //
