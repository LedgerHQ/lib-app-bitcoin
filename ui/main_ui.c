...
static void prepare_single_output(void) {
  ...
  get_address_from_output_script(context.currentOutput + offset,
                                 sizeof(context.currentOutput) - offset, tmp,
                                 sizeof(tmp));
  ...
  if ((context.currentOutput[offset + 2] == 0x14) &&
    ...
    uint32_t omniAssetId =
        read_u32_be(context.currentOutput, offset + 3 + 4 + 4);
    switch (omniAssetId) {
    case OMNI_ASSETID:
    ...
    case MAIDSAFE_ASSETID:
      ...
      break;
    default:
      snprintf(vars.tmp.fullAmount, sizeof(vars.tmp.fullAmount),
               "OMNI asset %d ", omniAssetId);
      break;
    }
    format_sats_amount(
        vars.tmp.fullAmount,
        (uint64_t)read_u64_be(context.currentOutput,
                              offset + 3 + 4 + 4 +
                                  4), // Cast prevents weird compilo bug
        vars.tmp.fullAmount);
  } else {
    format_sats_amount(
        COIN_COINID_SHORT,
...
static uint8_t prepare_message_signature(void) {
  ...
  format_hex((const uint8_t *)buffer, sizeof(buffer), vars.tmp.fullAddress,
...
unsigned int silent_confirm_single_output() {
  ...
  while (true) {
    ...
    memmove(context.currentOutput, context.currentOutput + context.discardSize,
...
uint8_t set_key_path_to_display(const unsigned char *keyPath) {
  format_path(keyPath, vars.tmp_warning.derivation_path,
...
void request_change_path_approval(unsigned char *change_path) {
  format_path(change_path, vars.tmp_warning.derivation_path,
              sizeof(vars.tmp_warning.derivation_path));
