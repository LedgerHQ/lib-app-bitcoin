...
#define IS_INPUT()                                                             \
  (buffer->size - 1 > 8 && buffer->size - 1 <= TRUSTED_INPUT_TOTAL_SIZE + 2 && \
   buffer->ptr[0] <= 0x02)
...
WEAK unsigned short handler_hash_input_start(buffer_t *buffer, uint8_t p1,
  ...
  if ((p2 == P2_NEW) || (p2 == P2_NEW_SEGWIT) ||
      (p2 == P2_NEW_SEGWIT_CASHADDR) || (p2 == P2_NEW_SEGWIT_OVERWINTER) ||
    ...
    if (p1 == P1_FIRST) {
      ...
      if ((COIN_KIND == COIN_KIND_ZCASH) || (COIN_KIND == COIN_KIND_KOMODO) ||
        ...
        } else if (p2 == P2_NEW_SEGWIT_SAPLING) {
  ...
  if (context.usingSegwit && !context.segwitWarningSeen && (p1 == P1_NEXT) &&
      (p2 != P2_CONTINUE)
      // ...if input is not passed as a TrustedInput
      && IS_INPUT() && !IS_INPUT_TRUSTED()) {
